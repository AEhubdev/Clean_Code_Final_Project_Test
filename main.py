...
def render_chart_window(title, chart_type, key_id, default_idx=2):
    with st.container(border=True):
        ...
        selected_tf = select_col.selectbox("TF", list(config.TIMEFRAME_OPTIONS.keys()), index=default_idx,
                                           key=f"sel_{key_id}", label_visibility="collapsed")

        df_for_chart, _, _, _ = data_engine.get_gold_market_data(selected_tf)
        plot_data = df_for_chart.tail(150)
        if plot_data.empty: return st.warning("Loading...")
        ...
        fig.update_layout(template="plotly_dark", margin=dict(t=30 if show_leg else 5, b=5, l=5, r=5),
                          showlegend=show_leg)
...
def _plot_price_with_signals(fig, data):
    ...
    if not predict_df.empty:
        fig.add_scatter(x=predict_df.index, y=predict_df['Predicted'],
                        line=dict(color='#FFD700', width=3, dash='dashdot'), name="AI 30D Forecast")

    # 1. Trend Line
    trend_vals = calculate_trend_line(data['Close'].values)
    fig.add_scatter(x=data.index, y=trend_vals, line=dict(color='orange', width=2, dash='dot'), name="Linear Trend")
    ...
    fig.add_scatter(x=data.index, y=data['BB_L'], line=dict(color='rgba(255,255,255,0.15)', width=1), fill='tonexty',
                    fillcolor='rgba(255,255,255,0.05)', name="BB Lower")
    ...
    fig.add_scatter(x=buys.index, y=buys['Low'] - off, mode='markers',
                    marker=dict(symbol='triangle-up', size=14, color='#00FF41'), name="Buy Signal")
    fig.add_scatter(x=sells.index, y=sells['High'] + off, mode='markers',
...
def _render_news_section(news_list):
    ...
    if not news_list: return st.info("No news.")
