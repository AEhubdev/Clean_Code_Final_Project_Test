...
def render_live_overview():
    ...
    with col_sidebar:
        ...
        c1, c2 = st.columns(2)
        r_val = latest['RSI']
        r_col = "red" if r_val > 70 else "green" if r_val < 45 else "#00d4ff"
        c1.markdown(
            f'<div style="background:#1e2130; padding:10px; border-radius:5px; border-left:4px solid {r_col}"><small>RSI (14)</small><br><strong>{r_val:.1f}</strong></div>',
            unsafe_allow_html=True)

        m_val = latest['MACD_Hist']
        m_dir, m_color = ("UP", "green") if m_val > 0 else ("DOWN", "red")
        c2.markdown(
            f'<div style="background:#1e2130; padding:10px; border-radius:5px; border-left:4px solid {m_color}"><small>MACD Hist</small><br><strong style="color:{m_color}">{m_dir}</strong></div>',
            unsafe_allow_html=True)
...
def render_window(title, chart_type, key_id, default_idx=2):
    with st.container(border=True):
        ...
        if data.empty: return st.warning("Syncing timeframe...")
