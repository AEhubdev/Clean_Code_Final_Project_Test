...
def render_live_overview():
    ...
    with col_sidebar:
        ...
        # --- SIDEBAR METRICS RESTORED ---
        st.markdown("---")
        c1, c2 = st.columns(2)
        r_val = latest['RSI']
        r_col = "red" if r_val > 70 else "green" if r_val < 30 else "#00d4ff"
        c1.markdown(f'<div style="background:#1e2130; padding:10px; border-radius:5px; border-left:4px solid {r_col}"><small style="color:gray">RSI (14)</small><br><strong style="font-size:18px">{r_val:.1f}</strong></div>', unsafe_allow_html=True)

        m_dir, m_col = ("UP", "green") if latest['MACD_Hist'] > 0 else ("DOWN", "red")
        c2.markdown(f'<div style="background:#1e2130; padding:10px; border-radius:5px; border-left:4px solid {m_col}"><small style="color:gray">MACD</small><br><strong style="font-size:18px; color:{m_col}">{m_dir}</strong></div>', unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        sk, sd = latest['Stoch_K'], latest['Stoch_D']
        s_col = "green" if sk > sd else "red"
        st.markdown(f'<div style="background:#1e2130; padding:12px; border-radius:5px;"><div style="display:flex; justify-content:space-between"><span style="color:gray">Stoch (K/D)</span><span style="color:{s_col}; font-weight:bold">{"Bullish" if sk > sd else "Bearish"}</span></div><div style="font-size:20px; font-weight:bold">{sk:.0f} / {sd:.0f}</div></div>', unsafe_allow_html=True)
...
def render_window(title, chart_type, key_id, default_idx=2):
    with st.container(border=True):
        ...
        tf = s_col.selectbox("TF", list(config.TIMEFRAME_OPTIONS.keys()), index=default_idx, key=f"sel_{key_id}", label_visibility="collapsed")

        full_df, _, _, _ = data_engine.get_gold_data(tf)
        lookback = 100 if tf in ["15m", "1h"] else 250
        data = full_df.tail(lookback)
        if data.empty: return st.warning("Loading Data...")

        fig = go.Figure()
        if chart_type == "price":
            # Dynamic Signal Offset Fix
            ...
            try:
                ...
                fig.add_trace(go.Scatter(x=data.index, y=trend, line=dict(color='orange', width=2, dash='dot'), name="Trend"))
            except: pass
            ...
            fig.add_trace(go.Scatter(x=data.index, y=data['BB_U'], line=dict(color='rgba(173, 216, 230, 0.2)', width=1), name="BB Up"))
            fig.add_trace(go.Scatter(x=data.index, y=data['BB_L'], line=dict(color='rgba(173, 216, 230, 0.2)', width=1), fill='tonexty', name="BB Low"))

            # Candles
            fig.add_trace(go.Candlestick(x=data.index, open=data['Open'], high=data['High'], low=data['Low'], close=data['Close'], name="Price"))

            # Signals Restored with Layering & Offset Fix
            ...
            fig.add_trace(go.Scatter(x=buys.index, y=buys['Low'] - offset, mode='markers',
                marker=dict(symbol='triangle-up', size=15, color='#00FF41', line=dict(width=1, color='white')), name="BUY"))
            fig.add_trace(go.Scatter(x=sells.index, y=sells['High'] + offset, mode='markers',
                marker=dict(symbol='triangle-down', size=15, color='#FF3131', line=dict(width=1, color='white')), name="SELL"))
            ...
            fig.add_bar(x=data.index, y=data['MACD_Hist'], marker_color=m_colors)
            fig.update_layout(height=200)
